...
// ─── Tool schemas (Zod) ─────────────────────────────────────────────────
const slugArg = (name) => DEFAULT_PROJECT_SLUG
...
const Tools = {
    ...
    get_page: {
        ...
        handler: async (args) => {
            ...
            const { data: page } = await supabase
                ...
                .single();
            if (!page)
                throw new Error(`Page not found: ${args.page_slug}`);
    ...
    update_page_status: {
        ...
        handler: async (args) => {
            ...
            const { data: page, error } = await supabase
                ...
                .single();
            if (error || !page)
                throw new Error(error?.message ?? `Page not found: ${args.page_slug}`);
            await logActivity({
    ...
    update_section_status: {
        ...
        handler: async (args) => {
            const { data: section, error } = await supabase
                ...
                .single();
            if (error || !section)
                throw new Error(error?.message ?? `Section not found: ${args.section_id}`);
    ...
    list_designs: {
        ...
        handler: async (args) => {
            ...
            if (args.viewport)
                q = q.eq('viewport', args.viewport);
            if (pageIdFilter)
                q = q.eq('page_id', pageIdFilter);
            const { data, error } = await q;
            if (error)
                throw error;
            return ok(data ?? []);
...
function zodFieldToJsonSchema(value) {
...
// ─── Server ─────────────────────────────────────────────────────────────
const server = new Server({ name: 'elevate-control', version: '0.1.0' }, { capabilities: { tools: {} } });
